...
or LL, your `seq_config.h` settings, and the smallest piece of code that shows
the problem. A failing test is even better, see below.

## Making a change

1. Add or update a test in `test/test_seq.c` that fails before your change and
...
3. Match the existing code style. The short version: 4 spaces and no tabs, Allman
   braces, `snake_case`, every file scope name prefixed with `seq_`, a Doxygen
   ...
   ```bash
   clang-format -i src/seq.c src/seq.h
   ```

4. Keep the public header free of vendor headers. `src/seq.h` includes only
   `<stdint.h>` and `seq_config.h`, which is what lets the tests run on a PC.
   Anything from the HAL belongs in `src/seq.c`.
...
- a cross compile of `src/seq.c` for Cortex-M4 with the same warning settings
